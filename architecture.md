...
- On each drag and resize, we push up the changes to keep the global `refMap` in sync. In other words, each `RefImage` *pulls* updates from `refMap` on render but also *pushes* updates if "directly" manipulated.
    - This two-way syncing enables multi-selection operations (e.g. dragging three selected images at once) by updating the `refMap` entries of all selected images, manipulating them "indirectly".
